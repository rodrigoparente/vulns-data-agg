...
def process_security_feeds():
    ...
    microsoft_cves = microsoft_advisory['cve_id'].tolist()
    intel_cves = intel_advisory['cve_id'].tolist()
    adobe_cves = adobe_advisory['cve_id'].tolist()

    # remove intersection between microsoft and intel if any
    intersection = set(microsoft_cves).intersection(set(intel_cves))
    intel_advisory = intel_advisory.loc[~intel_advisory['cve_id'].isin(intersection)]

    # remove intersection between microsoft and intel if any
    intersection = set(microsoft_cves).intersection(set(adobe_cves))
    adobe_advisory = adobe_advisory.loc[~adobe_advisory['cve_id'].isin(intersection)]

    # remove intersection between microsoft and intel if any
    intersection = set(intel_cves).intersection(set(adobe_cves))
    adobe_advisory = adobe_advisory.loc[~adobe_advisory['cve_id'].isin(intersection)]
    ...
    cves = cves[[
        'cve_id', 'part', 'vendor', 'base_score', 'confidentiality_impact', 'integrity_iImpact',
    ...
    cves.to_csv('output/vulns.csv', index=False)
